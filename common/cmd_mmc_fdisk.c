...
#define		DISK_START			(16*1024*1024)   //same as raw area size
...
/////////////////////////////////////////////////////////////////
int make_mmc_partition(u64 total_block_count, unsigned char *mbr, int flag, char *argv[])
{
...
/////////////////////////////////////////////////////////////////
u64 get_mmc_block_count(char *device_name)
{
	struct mmc *mmc;
	u64 block_count = 0;
	int dev_num;
	...
	mmc = init_mmc_device(dev_num, false);
	if (!mmc)
		return -1;

	block_count = mmc->capacity / BLOCK_SIZE;
	printf("mmc->capacity = %llu , mmc->read_bl_len = %d\n",mmc->capacity,mmc->read_bl_len);
...
int put_mmc_mbr(unsigned char *mbr, char *device_name)
{
	...
	if (mmc_getwp(mmc) == 1) {
		printf("Error: 这个设备是只读的！\n");
		return -1;
	}
...
int create_mmc_fdisk(int argc, char *argv[])
{
	...
	{
		printf("get_mmc_block_count err\n");
		return -1;
	}
	...
	argv[1][1] = 'p';
	if(print_mmc_part_info(argc, argv) == -1)
		return -1;
	return 0;
}
...
/////////////////////////////////////////////////////////////////
int do_fdisk(cmd_tbl_t *cmdtp, int flag, int argc, char * const cargv[])
{
	char **argv;
	argv = (char **)cargv;
	printf("argc = %d\n",argc);
